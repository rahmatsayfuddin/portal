...
public function register_clm()
{
    $param['username']=$_POST['username'];
    $param['nama_depan']=$_POST['nama_depan'];
    $param['nama_belakang']=$_POST['nama_belakang'];
    $param['email']=$_POST['email'];
    $param['password']=$_POST['password'];
    //echo json_encode($param);
    $hasil= $this->Home->register_clm($param);
    if (isset($hasil['errorID']))  {
        $this->session->set_flashdata('msgdaftar', 
            '<div class="alert alert-danger">
            <h4>Maaf Anda Gagal Mendaftar</h4>
            </div>');    
