...
function TotalSemua()
{
    //Here we get the total price by calling our function
    //Each function returns a number so by calling them we add the values they return together
    var pricepack = parseInt($('#packprice').val());
    var cakePrice = getDatabasePrice() + getFillingPrice() + vlansPrice() + vpnsPrice() + loadsPrice() + firewallsPrice() +$total_all +pricepack;
